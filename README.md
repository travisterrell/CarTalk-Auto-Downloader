### I forked an old archived github project and fixed it. Also improved it a bit to use the episode number and title for the file names, since the downloaded file names are not descriptive. It's unfortunate that NPR limits the number of past episodes in the podcast XML, but this is a solid workaround. My changes aren't pretty or anything—just getting it going for a one-time use of downloading the archive, but they work! (Currently!) 
